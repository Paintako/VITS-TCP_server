...
def inference(phoneme_ids,  speaker_id, NET_G, HPS):
    if str(speaker_id) in infer_speedDict:
        infer_speed = infer_speedDict[speaker_id]
    else:
        infer_speed = 1.0
    result_np_arr = []
    for id in phoneme_ids:
        ...
        with torch.no_grad():
            ...
            audio = NET_G.infer(x_tst, x_tst_lengths, sid=sid, noise_scale=0.5, noise_scale_w=0.5, length_scale=infer_speed)[0][0,0].data.cpu().float().numpy()
